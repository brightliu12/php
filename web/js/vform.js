...
(function($) {
	$.fn.vForm = function(options) {
		...
		return this.each(function() {
			...
			// 后续可添加一些重置，验证之类的命令
			/*
			 * if ($('#login').vForm('validate')) {
			 *  } switch (command) { case 'init': break; case 'validate':
			 * break;default: }
			 */
			return this;
		});
		...
		ajax : function(value, rule) {
			...
			$.post(rule.url, {
				...
				if (data.value == $field.val()) {
					if (data.isValid) {
						// 验证通过
						$field.data('lastStatus', 'success');
						vField.setStatus('success');
					} else {
						// 验证不通过
						$field.data('lastStatus', 'error');
						var errorMsg;
						if (rule['errorMsg']) {
						...
						vField.setStatus('error', errorMsg);
					}
				}
	...
	/*
	 * 字段原型属性和方法，isValid用于判断是否通过验证；setStatus用于设置bootstrao样式
	 */
	...
	$.fn.vForm.field.prototype = {
		isValid : function() {
			...
			if (this.rules.length) {
				...
				validateRules: for (i in this.rules) { // 调用验证方法
					...
					switch ($.fn.vForm.validator[rule.v].call(this, value, rule)) {
					case false:
					case 'error':
						// Error 必须放在最前面
						isValid = 'error';
		...
		setStatus : function(status, msg) {
			if (status == this.status)
				return true;
			var $elm = this.$field.parents('.control-group');
			if (this.status) {
				$elm.removeClass(this.status); // 变颜色
			}

			if (status) {
				$elm.addClass(status); // 变颜色
			}
			this.status = status;

			if (msg) {
				this.$field.nextAll('.vform-help').html(msg); // 输出错误信息
			} else if (this[status + 'Msg']) {
				this.$field.nextAll('.vform-help').html(this[status + 'Msg']);
			} else {
				this.$field.nextAll('.vform-help').html('');
			}
		},
		successMsg : '输入正确',
		errorMsg : '输入错误',
		...
		error : function(data) {
			if (!data.isValid) {
				for ( var i in data.error) {
					this.$form.find('[name=' + i + ']:eq(0)').data('vField')
							.setStatus('error', data.error[i]);
				}
			}
		}
	};
